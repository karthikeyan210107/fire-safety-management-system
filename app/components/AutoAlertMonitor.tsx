...
function SensorPoint({
  ...
  useFrame(({ clock }) => {
    ...
    const pulse =
      1 + Math.sin(clock.elapsedTime * speed) * 0.18;
...
function Building({
  ...
  const dangerExists = zones.some(
    (zone) => zone.status === "DANGER"
  );

  const warningExists = zones.some(
    (zone) => zone.status === "WARNING"
  );

  const buildingColor = dangerExists
    ? "#261014"
    : warningExists
      ? "#211d0d"
      : "#111923";

  const buildingEdgeColor = dangerExists
    ? "#ff3030"
    : warningExists
      ? "#ffd43b"
      : "#1ddcff";
...
export default function CommandCenter3D() {
  ...
  useEffect(() => {
    const temperatureRef =
      ref(database, "test/value");

    const gasRef =
      ref(database, "sensors/gas/value");

    const flameRef =
      ref(database, "sensors/flame/value");
